...
const featureNameToTemplateModuleInternal = async (devserver, featureName) => {
  ...
  for (const filename of directoryFilenames) {
    ...
    const templateModule = await loadTemplateModule(devserver, templateFilepath);
    const templateModuleInternal = convertTemplateModuleToTemplateModuleInternal(templateFilepath, templateModule, false);
    if (featureName === normalizeTemplateName(templateModuleInternal.config.name)) {

...
const serverRenderRoute = ({ vite, dynamicGenerateData, projectStructure }) => async (req, res, next) => {
  try {
    ...
    const templateModuleInternal = await featureNameToTemplateModuleInternal(vite, feature);
    ...
    const featuresConfig = convertTemplateConfigInternalToFeaturesConfig(templateModuleInternal.config);
    ...
    const appHtml = await ReactDOMServer.renderToString(React.createElement(Component, props));
    ...
    const html = template.replace(`<!--app-html-->`, appHtml).replace(`<!--app-head-->`, `<head>

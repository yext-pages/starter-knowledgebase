...
const reactWrapper = (props, templateModuleInternal, template, hydrate, getHeadConfig) => {
  ...
  return `<!DOCTYPE html>
    ...
    <head>
        ...
        ${getCssTags(`${projectFilepaths.templatesRoot}/${templateModuleInternal.templateName}.tsx`, props.__meta.manifest.bundlerManifest, /* @__PURE__ */ new Set()).map((f) => `<link rel="stylesheet" href="/${f}"/>`).filter((v, i, a) => a.indexOf(v) == i).join("\n")}
        ${headConfig ? renderHeadConfigToString(headConfig) : ""}
    </head>
    <body>
        <div id="reactele">${template}</div>${hydrate ? `<script type="module" src="/${findHydrationFilename(`${projectFilepaths.hydrationBundleOutputRoot}/${templateModuleInternal.templateName}.tsx`, props)}" defer><\/script>` : ""}
    </body>
    </html>`;
};

...
const readTemplateModules = async (feature, manifest) => {
  ...
  const templateModuleInternal = convertTemplateModuleToTemplateModuleInternal(path, importedModule, true);
...
const renderHtml = (templateModuleInternal, props) => {
  ...
  if (!component && !render) {
    throw new Error(`Cannot render html from template '${templateModuleInternal.config.name}'. Template is missing render function or default export.`);
  }
  if (render) {
    if (getHeadConfig) {
      console.warn(`getHeadConfig for template ${templateModuleInternal.config.name} will not be called since a custom render function is defined.`);
    }
    return render(props);
  }
  return reactWrapper(props, templateModuleInternal, renderToString(createElement(component, props)), true, getHeadConfig);
};
export {
  generateResponses,
  readTemplateModules
